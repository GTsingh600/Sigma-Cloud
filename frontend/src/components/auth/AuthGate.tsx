...
export default function AuthGate({ children }: { children: React.ReactNode }) {
  const { isAuthenticated, loading, backendWaking } = useAuth();

  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#161d23] px-6 text-[#f8f4ef]">
        <div className="sigma-card w-full max-w-md text-center animate-in">
          ...
          <p className="mt-2 text-sm text-sigma-500">
            Hold on while SigmaCloud validates your Google login.
          </p>
          <ColdStartNotice active={backendWaking} />
        </div>
      </div>
    );
  }
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#161d23] px-6 text-[#f8f4ef]">
        <div className="sigma-card w-full max-w-lg animate-in">
          ...
          <p className="mt-3 max-w-md text-sm leading-6 text-sigma-500">
            Google authentication protects datasets, training jobs, deployed
            models, and predictions per user.
          </p>
          <div className="mt-6">
            <GoogleSignInButton />
          </div>
